...
For example, this might cause lead to issues with mismatches between an attribute mapped as a `org.hibernate.type.DateTypeImpl` and an attribute defined as a `org.hibernate.type.TimestampTypeImpl`,
