...
jts_geometry::
	Handled by `org.hibernate.spatial.JTSGeometryTypeImpl` it maps a database geometry column type to a `com.vividsolutions.jts.geom.Geometry` entity property type.
geolatte_geometry::
	Handled by `org.hibernate.spatial.GeolatteGeometryTypeImpl`, it maps a database geometry column type to an `org.geolatte.geom.Geometry` entity property type.
