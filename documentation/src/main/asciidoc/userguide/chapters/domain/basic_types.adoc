...
These mappings are managed by a service inside Hibernate called the `org.hibernate.type.BasicTypeRegistry`, which essentially maintains a map of `org.hibernate.type.spi.BasicType` (a `org.hibernate.type.Type` specialization) instances keyed by a name.
...
how did Hibernate know to use its `org.hibernate.type.StringTypeImpl` for mapping for `java.lang.String` attributes,
or its `org.hibernate.type.IntegerTypeImpl` for mapping `java.lang.Integer` attributes?

The answer lies in a service inside Hibernate called the `org.hibernate.type.BasicTypeRegistry`, which essentially maintains a map of `org.hibernate.type.spi.BasicType` (a `org.hibernate.type.Type` specialization) instances keyed by a name.
