...
public class XmlTest extends BaseEntityManagerFunctionalTestCase {
	@Test
	public void testXmlMappingCorrectness() {
		EntityManager em = getOrCreateEntityManager();
		em.close();
	}

	@Test
	public void testXmlMappingWithCacheable() {
		EntityManager em = getOrCreateEntityManager();
		SharedSessionContractImplementor session = em.unwrap( SharedSessionContractImplementor.class );
		EntityDescriptor entityDescriptor= session.getFactory().getMetamodel().getEntityDescriptor( Lighter.class.getName() );
		Assert.assertNotNull(entityDescriptor.getHierarchy().getEntityCacheAccess());
	}
