...
public class BlobDescriptorTest extends AbstractDescriptorTest<Blob> {
	...
	public BlobDescriptorTest() {
		super( BlobJavaDescriptor.INSTANCE );
	}
	...
	public void testEquality() {
		...
		assertTrue( BlobJavaDescriptor.INSTANCE.areEqual( original, original ) );
		assertFalse( BlobJavaDescriptor.INSTANCE.areEqual( original, copy ) );
		assertFalse( BlobJavaDescriptor.INSTANCE.areEqual( original, different ) );
	}
	...
	public void testExternalization() {
		// blobs of the same internal value are not really comparable
		String externalized = BlobJavaDescriptor.INSTANCE.toString( original );
		Blob consumed = BlobJavaDescriptor.INSTANCE.fromString( externalized );
		try {
			PrimitiveByteArrayJavaDescriptor.INSTANCE.areEqual(
