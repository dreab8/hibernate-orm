...
@Jpa(
		annotatedClasses = {
				DateTimeParameterTest.Thing.class
		}
)
public class DateTimeParameterTest {
	...
	public void testBindingCalendarAsDate(EntityManagerFactoryScope scope) {
		scope.inTransaction(
				entityManager -> {
					Query query = entityManager.createQuery( "from Thing t where t.someDate = :aDate" );
	...
	public void testBindingNullNativeQueryPositional(EntityManagerFactoryScope scope) {
		scope.inTransaction(
				entityManager -> {
					final Query query = entityManager.createNativeQuery(
							"update Thing set someDate = ?1 where id = 1" );
		...
		scope.inTransaction(
				entityManager -> {
					final Query query = entityManager.createNativeQuery( "select * from Thing where id = 1",
							Thing.class );
					List<Thing> list = query.getResultList();
					assertThat( list.size() ).isEqualTo( 1 );
					assertThat( list.get( 0 ).someDate ).isNull();
				}
		);
	}
	...
	public void testBindingNullNativeQueryNamed(EntityManagerFactoryScope scope) {
		scope.inTransaction(
				entityManager -> {
					final Query query = entityManager.createNativeQuery(
							"update Thing set someDate = :me where id = 1" );
	...
	public void createTestData(EntityManagerFactoryScope scope) {
		scope.inTransaction(
				entityManager -> entityManager.persist( new Thing( 1, "test", now, now, now ) )
		);
	}
