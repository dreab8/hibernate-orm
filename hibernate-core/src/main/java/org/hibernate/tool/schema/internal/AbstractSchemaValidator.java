...
public abstract class AbstractSchemaValidator implements SchemaValidator {
	...
	protected void validateColumnType(
			...
			ExecutionOptions options,
			Dialect dialect) {
		boolean typesMatch = column.getSqlTypeDescriptor().getSqlType() == columnInformation.getTypeCode()
		...
		if ( !typesMatch ) {
			throw new SchemaManagementException(
					String.format(
							...
							JdbcTypeNameMapper.getTypeName( column.getSqlTypeDescriptor().getSqlType() )
					)
			);
		}
