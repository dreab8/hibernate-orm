	...
	private void dropFromCreationContext(
		...
		for ( Namespace namespace : databaseModel.getNamespaces() ) {
			...
			for(Table table : namespace.getTables()){
				if(table.isExportable() && schemaFilter.includeTable( (ExportableTable) table ) ){
					...
					applySqlStrings(
							dialect.getTableExporter()
									.getSqlDropStrings( exportableTable, jdbcServices ),
							formatter,
							options,
							targets
					);
			...
			for ( Sequence sequence : namespace.getSequences() ) {
				...
				applySqlStrings( dialect.getSequenceExporter().getSqlDropStrings( sequence, jdbcServices ), formatter, options, targets );
			}
		}
