	...
	private static class BasicTypeResolverExplicitImpl implements BasicTypeResolver {
		...
		public <T> BasicType<T> resolveBasicType() {
			...
			if ( typeDefinition != null ) {
				...
				BasicType basicType = buildingContext.getBootstrapContext().getTypeConfiguration()
						.getBasicTypeRegistry().getBasicType( name );
				if ( basicType == null ) {
					return byReflection( name );
				}
				return basicType;
			}
		}

		protected BasicType byReflection(String typeName) {
			try {
				Class clazz = ReflectHelper.classForName( typeName );
				if ( org.hibernate.type.Type.class.isAssignableFrom( clazz ) ) {
					return (BasicType) clazz.newInstance();
				}
				else {
					throw new MappingException( "Could not instantiate Type: " + typeName );
				}
			}
			catch (Exception e) {
				throw new MappingException( "Could not instantiate Type: " + typeName, e );
			}
		}
	}
