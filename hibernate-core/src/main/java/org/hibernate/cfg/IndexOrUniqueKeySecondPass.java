...
public class IndexOrUniqueKeySecondPass implements SecondPass {
	...
	private void addConstraintToColumn(final String columnName ) {
		...
		if ( unique ) {
			table.getOrCreateMappedUniqueKey( indexName ).addColumn( (Column) column );
		}
		else {
			table.getOrCreateMappedIndex( indexName ).addColumn( (Column) column );
		}
	}

	private void addConstraintToColumns(List<Column> columns) {
		if ( unique ) {
			MappedUniqueKey uniqueKey = table.getOrCreateMappedUniqueKey( indexName );
			for ( Column column : columns ) {
				uniqueKey.addColumn( column );
			}
		}
		else {
			MappedIndex index = table.getOrCreateMappedIndex( indexName );
			for ( Column column : columns ) {
