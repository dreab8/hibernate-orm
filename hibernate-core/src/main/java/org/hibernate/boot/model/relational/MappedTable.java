...
public interface MappedTable<T extends MappedColumn> extends Loggable {
	...
	MappedIndex getOrCreateMappedIndex(String indexName);

	MappedUniqueKey getOrCreateMappedUniqueKey(String keyName);

	default MappedUniqueKey createMappedUniqueKey(List<T> columns){
		return null;
	}
	 ...
	 */
	MappedForeignKey createMappedForeignKey(
			String keyName,
			List<T> keyColumns,
	...
	Collection<MappedIndex> getMappedIndexes();

	Collection<MappedForeignKey> getMappedForeignKeys();

	Collection<MappedUniqueKey> getMappedUniqueKeys();

	MappedPrimaryKey getMappedPrimaryKey();
