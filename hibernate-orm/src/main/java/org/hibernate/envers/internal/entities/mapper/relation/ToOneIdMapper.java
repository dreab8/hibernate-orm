		...
		if ( entityId != null ) {
			if ( versionsReader.getFirstLevelCache().contains( referencedEntityName, revision, entityId ) ) {
				...
				if ( !referencedEntity.isAudited() ) {
					final String referencingEntityName = versionsReader.getAuditService().getEntityBindings().getEntityNameForVersionsEntityName( (String) data.get( "$type$" ) );
					if ( referencingEntityName == null && primaryKey == null ) {
						...
						ignoreNotFound = versionsReader.getAuditService().getEntityBindings().getRelationDescription( referencingEntityName, getPropertyData().getName() ).isIgnoreNotFound();
					}
				}
